...
exposed_combined_df.drop(['masterpatientid_y'], axis=1,inplace=True)
exposed_combined_df.rename(columns = {'masterpatientid_x':'masterpatientid'}, inplace = True)
...
unexposed_combined_df.replace(field_mappings, inplace=True)
        # return exposed_combined_df,unexposed_combined_df
...
def show_utilization_dashboard():
    ...
    redio2 = st.radio(
     "",
     ('Home', 'utilization analysis',"utilization dashboard"),
     horizontal=True,
     index=2)
    ...
    if redio2 == 'Home':
       ...
       st.experimental_rerun()
    elif redio2 == 'utilization analysis':
       st.session_state.current_page = 'Utilization Analysis'
       st.experimental_rerun()
    else:
...
def show_utilization_analysis():
    ...
    with col2:
        ...
        key="dropdown",label_visibility="hidden"
    )
